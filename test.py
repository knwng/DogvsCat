...
# metric_acc = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
...
with tf.Session() as sess:
    ...
    try:
        while not coord.should_stop():
            ...
            # _acc, _precision, _recall = sess.run([metric_acc, metric_precision, metric_recall], feed_dict={images: _images, labels: _labels})
            sess.run([op_acc, op_precision, op_recall], feed_dict={images: _images, labels: _labels})
            tf.logging.info("Evaluating Step: {}".format(tf.train.get_global_step().eval()))
